        ...
        return {
            initialProperties: {
                ...
                qHyperCubeDef: {
                    qDimensions: [],
                    qMeasures: [],
                    // Guntan: Values below changed to allow more dimensions
                    qInitialDataFetch: [{
                        qWidth: 20,
                        qHeight: 500
                    }]
            ...
            definition: {
                ...
                items: {
                    dimensions: {
                        uses: "dimensions",
                        min: 2,
                        //Guntan: Value below changed to allow more dimensions
                        max: 20
                    },
                    measures: {
                        uses: "measures",
            ...
            paint: function($element, layout) {
                ...
                this.backendApi.eachDataRow(function(rownum, row) {
                    ...
                    $.each(row, function(key, cell) {
                        if(k<row.length-1)
                        {
                            path += sep + (cell.qText.replace('|', ' ')) + '|' + (cell.qElemNumber);
                            sep = "§";
                        }
                        k++;
                    });
                    ...
                    source.push({
                        "Path": path,
                        "Frequency": row[row.length - 1].qNum
                    })
                });
                ...
                link.on("click", function(d, i) {
                    _this.backendApi.selectValues(
                        parseInt(d.target.name.split('~')[1].replace('end', qDim.length - 1)), [parseInt(d.target.name.split('~')[0].split('|')[1])],
                        true
                    );
                    _this.backendApi.selectValues(
                ...
                node.on("mouseover", function(d) {
                        ...
                        //Last nodes might not have enough space for the tooltip, so we add a negative margin to push the tooltip on the left
                        if (level === "e") {
                            level = qDim.length - 1;
                            edgeMargin = -80;
                        }
